...
def main(config_kwargs):
    ...
    if load_model:
        ...
        load_path = os.path.join(checkpoint_folder, load_file[-1])
        (
            encoder,
            predictor,
            target_encoder,
    ...
    def _load_snapshot(snapshot_path):
        ...
        predictor.load_state_dict(snapshot['PREDICTOR'])
        target_encoder.load_state_dict(snapshot['TARGET_ENCODER'])
    ...
    predictor = predictor.to(rank)
    predictor = DDP(predictor, device_ids = [rank])

    target_encoder = target_encoder.to(rank)
    target_encoder = DDP(target_encoder, device_ids = [rank])
    #predictor.to(gpu_id)
